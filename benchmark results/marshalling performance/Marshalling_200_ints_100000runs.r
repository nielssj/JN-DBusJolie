...
plot(sortedDbus, dnorm(sortedDbus, mean=dmean,sd=dsd), ylim=c(0,8), xlim=c(-.3,5),type="l", col="red",ann=F)
...
title(main="Marshalling 200 ints, 100.000 runs", xlab="ms")

text(-.1,8, expression("m"["D-Bus"]))
text(3,1.7, expression("m"["SOAP"]))
text(2,1, expression("3 SD"["SOAP"]))
